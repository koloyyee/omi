...
public class Benzinga implements Scrapable<TickerNews> {
  private static final String BASE_URL = "https://www.benzinga.com/quote/";
  private static final String END = "/news";
  private final Logger log = LoggerFactory.getLogger(Benzinga.class);
  ...
  private LocalDateTime recentPastToMins(String recentPast) {
    ...
    //            log.info(test);
    //            // handle month. e.g.
    //            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("MMMM d, uuuu, hh:mma",
    // Locale.ENGLISH);
    ...
    try {
      ...
      for (Element element : headlines) {
        ...
        String[] contentFooter =
            element.select("div.text-gray-400.text-sm").text().trim().split("-");
        ...
        TickerNews tickerNews =
            new TickerNews(ticker, title, href, Outlet.BENZINGA, source, issuedDate);
  ...
  /**
   * Reference to parallelStream and multithreaded scraping source:
   * https://stackoverflow.com/questions/63022139/web-scraping-using-multithreading/63022373#63022373
   */
